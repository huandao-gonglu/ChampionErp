...
- Product research AI web search is a first-class search provider strategy (`source_type: ai_search`, `config_json.provider_strategy: ai_web_search`). It resolves a model through `erp_web/services/ai_gateway.py`, can optionally pin `config_json.ai_model_id`, and streams model output tokens into the async run `description` while the run is still active.
