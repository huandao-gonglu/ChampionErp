...
def test_real_agent_function_model_round_trip_preserves_schema_and_tool_call_id() -> (
    None
):
    ...
    def model_function(
        ...
        if model_turns == 1:
            ...
            assert pydantic_definition.sequential is False
...
def test_bridge_routes_permission_denial_through_runtime_without_executor_call() -> (
    None
):
    executions = 0
...
def test_bridge_keeps_business_scope_out_of_model_schema_and_passes_it_to_executor() -> (
    None
):
    ...
    dependencies, runtime = bind_dependencies(
        toolset,
        execution_context(business_scope={"platform": "mercadolibre", "site": "MLM"}),
    )
    bridge = PydanticToolBridge(toolset)
...
def test_bridge_preserves_runtime_idempotency_only_for_identical_call_ids() -> None:
    executions: list[str] = []
...
def test_bridge_preserves_public_error_without_code_enumeration() -> None:
    ...
    result = execute_bridge(
        PydanticToolBridge(toolset), dependencies, call_id="public-failure"
    )
    assert result["ok"] is False
    assert result["error"]["code"] == "DOMAIN_CUSTOM_FAILURE"
    assert result["error"]["message"] == "领域服务暂时不可用。"
