...
# -- P2：resume_sync 走统一内核并发布 presentation 事件 ----------------------


class WriteSaved(BaseModel):
    model_config = ConfigDict(extra="forbid")

    saved: bool


WRITE_PROFILE = AiAgentExecutionProfile(
    use_case_id="presentation.kernel.write",
    output_type=WriteSaved,
    toolset_id="presentation.kernel.write",
    budget_profile="presentation.kernel.v1",
    permissions=frozenset({"presentation.write"}),
    approval_permission="presentation.approve",
    timeout_seconds=10,
    max_model_requests=4,
    max_tool_calls=2,
    max_tool_output_bytes=4096,
    retries=0,
    result_version="presentation_write.v1",
    allow_write=True,
)


def _write_toolset(executor) -> AiToolSet:
    definition = AiToolDefinition(
        name="save_note",
        version="1",
        description="保存一条笔记（写工具，需审批）",
        input_schema={
            "type": "object",
            "required": ["note"],
        ...
        required_permission="presentation.write",
        side_effect="write",
...
def _write_model(messages: list[Any], info: AgentInfo) -> ModelResponse:
    tool_returns = [
        part
        for message in messages
        if isinstance(message, ModelRequest)
    ...
    return ModelResponse(
        parts=[
            ToolCallPart(
                info.output_tools[0].name,
                {"saved": saved},
                tool_call_id="final-after-resume",
            )
        ]
    )


def test_resume_publishes_presentation_chunks_under_bound_scope(
    tmp_path: Path,
) -> None:
    """resume_sync 必须经统一 native-event 内核发布展示事件（P2）。"""

    executions = 0

    def executor(arguments: dict[str, Any], context: AiExecutionContext) -> Any:
        nonlocal executions
        assert arguments == {"note": "恢复后写入"}
        del context
        executions += 1
        return {"saved": True}
    ...
    factory = _factory(
        tmp_path,
        {WRITE_PROFILE.use_case_id: FunctionModel(_write_model)},
    )
    paused = factory.run_sync(
        profile=WRITE_PROFILE,
        instructions="调用写工具保存笔记。",
        user_prompt="保存一条笔记。",
        toolset=_write_toolset(executor),
        tenant_id="tenant-1",
        business_scope={"note_id": "note-1"},
        idempotency_context={"operation_id": "operation-1"},
    )
    ...
    registry = AiPresentationRegistry()
    presentation_id, scope = _reserve_and_claim(registry, title="AI 恢复审批")

    with bind_presentation_context(scope):
        resumed = factory.resume_sync(
            ...
            tenant_id="tenant-1",
            permissions={"presentation.write", "presentation.approve"},
            business_scope={"note_id": "note-1"},
            idempotency_context={"operation_id": "operation-1"},
        )
    assert resumed.output == WriteSaved(saved=True)
    assert executions == 1
    resumed.complete()
    registry.finish_request(presentation_id, request_failed=False)
    ...
    # 恢复运行发布了官方 chunk（统一内核），且以 finish 收尾。
    text = _drain_chunks(registry, presentation_id)
    frames = _parse_frames(text)
    assert frames, "resume 必须经统一内核发布 presentation 事件"
    assert any(frame.get("type") == "finish" for frame in frames)
    assert not [frame for frame in frames if frame.get("type") == "error"]

    # 恢复 root run 的规范历史落在 presentation conversation（真实 store）。
    assert resumed.conversation_id == scope.conversation_id
    history = factory.message_store.get(scope.conversation_id)
    assert history is not None
    assert history.messages_json == ModelMessagesTypeAdapter.dump_json(
        resumed.messages
