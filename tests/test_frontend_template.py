...
class FrontendTemplateTests(unittest.TestCase):
    ...
    def test_api_layer_is_centralized(self) -> None:
        ...
        workflow_domains = [
            (FRONT_SRC_DIR / "api" / "workflow" / f"{domain}.ts").read_text(
                encoding="utf-8"
            )
            for domain in ("state", "catalog", "publishing", "settings")
        ]
        ...
        self.assertNotIn("apiClient", workflow_api)
        self.assertTrue(all("apiClient" in module for module in workflow_domains))
        self.assertTrue(
            all("from '@/api/client'" in module for module in workflow_domains)
        )
        for domain in ("state", "catalog", "publishing", "settings"):
            self.assertIn(f"export * from './workflow/{domain}'", workflow_api)
        self.assertIn("/api/state", workflow_domains[0])
    ...
    def test_python_backend_prefers_built_vue_dist_and_serves_assets(self) -> None:
        ...
        self.assertFalse((APP_DIR / "erp_web" / "runtime_units" / "runtime_common.py").exists())
        self.assertIn("front_dist_dir: Path", context_py)
        self.assertIn("front_dist_index_path: Path", context_py)
