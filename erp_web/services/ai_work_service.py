"""AI Provider 对话事件的 JSONL journal。"""
...
_CONVERSATION_LOCKS: dict[str, threading.Condition] = {}


def _condition_for(conversation_id: str) -> threading.Condition:
    with _LOCKS_GUARD:
        return _CONVERSATION_LOCKS.setdefault(conversation_id, threading.Condition(threading.Lock()))
...
def ai_work_root(app_dir: Path | str) -> Path:
    return Path(app_dir) / AI_WORK_RELATIVE_DIR


def _conversation_path_for_date(app_dir: Path | str, conversation_id: str, date_text: str) -> Path:
    return ai_work_root(app_dir) / date_text / f"{_safe_conversation_id(conversation_id)}.jsonl"


def find_conversation_path(app_dir: Path | str, conversation_id: str) -> Path | None:
    safe_id = _safe_conversation_id(conversation_id)
    root = ai_work_root(app_dir)
    if not root.exists():
        return None
    matches = sorted(root.glob(f"*/{safe_id}.jsonl"), reverse=True)
...
class AiWorkConversation:
    ...
    def __post_init__(self) -> None:
        self._condition = _condition_for(self.conversation_id)
...
def start_conversation(
    ...
    *,
    ...
    path = _conversation_path_for_date(app_dir, conversation_id, now.strftime("%Y-%m-%d"))
    metadata = {
...
def load_conversation_events(
    ...
    *,
    ...
    path = find_conversation_path(app_dir, conversation_id)
    return _read_events_from_path(path, after_seq) if path else []


def wait_for_conversation_events(
    app_dir: Path | str,
    conversation_id: str,
    *,
    after_seq: int = 0,
    ...
    events = load_conversation_events(app_dir, conversation_id, after_seq=after_seq)
    if events or wait_ms <= 0:
        return events
    timeout = min(max(int(wait_ms), 0), MAX_WAIT_MILLISECONDS) / 1000
    condition = _condition_for(_safe_conversation_id(conversation_id))
...
def list_conversations(app_dir: Path | str, *, limit: int = 50) -> list[AiWorkConversationSummary]:
    safe_limit = min(max(int(limit), 1), MAX_CONVERSATION_LIST_LIMIT)
    root = ai_work_root(app_dir)
    if not root.exists():
        return []
    paths = sorted(root.glob("*/*.jsonl"), key=lambda path: path.stat().st_mtime, reverse=True)[:safe_limit]
    return [_summary_from_events(_read_events_from_path(path), path) for path in paths]
