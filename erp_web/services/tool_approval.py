...
digest 同时绑定 Capability 名称/版本、operation_key、原生 tool_call_id 与审批版本；
批准后的执行必须重算快照并复核 digest，任何受保护参数或审批版本变化都
会使旧审批失效。
"""
...
def verify_execution_approval(
    ...
    *,
    ...
    if not digest or execution.approval_revision <= 0:
        raise AiToolExecutionError(
            "TOOL_APPROVAL_CONTEXT_REQUIRED",
            "审批写入缺少可信审批上下文，不能执行。",
        )
    ...
    expected = approval_binding_digest(
        ...
        approval_revision=execution.approval_revision,
    )
    if expected != digest:
        raise AiToolExecutionError(
            stale_code,
            "审批内容与当前执行参数或审批版本已不一致，原审批已失效。",
        )
