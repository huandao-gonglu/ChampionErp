...
保存和读取均只使用官方 ``ModelMessagesTypeAdapter``。Deferred Tool 的开口
history（存在 ``ToolCallPart`` 但尚无 ``ToolReturnPart``）是合法且必须保留
的：读取时不合成任何 tool return，真正的闭合只能通过官方
``DeferredToolResults`` 的后续 run 完成。每次成功保存都会递增
``history_version``，供 Deferred link CAS 与事件订阅 cursor 使用。
"""
...
class PydanticMessageHistory:
    ...
    def model_messages(self) -> list[ModelMessage]:
        ...
        Deferred 开口保持官方消息原貌：不补齐、不合成任何 tool return。
        """

        return list(ModelMessagesTypeAdapter.validate_json(self.messages_json))


def canonical_model_messages_json(value: Any, *, stored: bool) -> bytes:
    """把消息序列/已存 BLOB 归一化为官方 adapter 的规范 JSON bytes。"""
...
def _history_from_row(row: dict[str, Any]) -> PydanticMessageHistory:
    try:
        ...
        messages_json = canonical_model_messages_json(
            row["messages_json"],
            stored=True,
        )
...
class PydanticMessageStore:
    ...
    def save(
        ...
        """以完整消息列表原子替换同一 conversation 的历史并递增版本。"""
    ...
    def get_version(self, conversation_id: str) -> int:
        """读取当前 history version；conversation 不存在时返回 0。"""

        return self.db.get_pydantic_message_history_version(
