...
<template>
  <section class="space-y-4">
    <div class="flex flex-wrap items-center justify-between gap-3"><div><h3 class="font-bold">发布 SKU · 已选择 {{ selectedCount }} / {{ enabledPairs.length }}</h3><p class="muted mt-1">启用规格默认全选，可取消不刊登的规格。修改只影响当前草稿。可售库存默认填写供应商库存，可手动调整。成本和包装资料默认跟随商品，展开一行可单独调整。</p></div><div class="flex gap-2"><button class="btn btn-outline" :disabled="loading" @click="setAllSelected(true)">全选启用规格</button><button class="btn btn-outline" :disabled="loading" @click="setAllSelected(false)">取消全选</button></div></div>
