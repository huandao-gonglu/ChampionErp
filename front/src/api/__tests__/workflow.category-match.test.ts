...
describe('category.product_match 同步 focused response + 通用 presentation 契约', () => {
  ...
  it('reserve 走通用 presentation endpoint，业务请求走同步 focused endpoint 并保留类型化结果 shape', async () => {
    ...
    expect(result.candidates[0]?.raw.aiRecommended).toBe(true)
    expect(result.candidates[0]?.raw.matchEvidence).toEqual(['主体一致'])
    expect(result.candidates[1]?.raw.aiRecommended).toBe(false)
